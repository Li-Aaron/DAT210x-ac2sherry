...
def do_PCA(armadillo):
  ...
  armadillo_pca = pca.transform(armadillo)
...
def do_RandomizedPCA(armadillo):
  ...
  armadillo_rpca = rpca.transform(armadillo)
  
  return armadillo_rpca
...
for i in range(100): pca = do_PCA(armadillo)
...
t1 = datetime.datetime.now()
for i in range(100): rpca = do_RandomizedPCA(armadillo)
time_delta = datetime.datetime.now() - t1
...
#直接投射看看啥样
#armadillo_yz = armadillo.drop('x',axis = 1)
fig = plt.figure()
ax = fig.add_subplot(111)
ax.scatter(-armadillo.z, armadillo.y, c='black', marker='.', alpha=0.75)
