...
def doKMeans(data, clusters=1, show = 1):
  #
  ...
  #
  # .. your code here ..
  data_loc = data.loc[:,['TowerLon','TowerLat']]
  model = KMeans(n_clusters=clusters)
  model.fit(data_loc)
  centroids = model.cluster_centers_
  if(show == 1):
#      assignment2result = [-96.70839404, 32.73299989]
      assignment2result = [-96.890472, 32.722042]
      # No.8: 2894365987 --- -96.89201378  32.72165711
      fig = plt.figure()
      ax = fig.add_subplot(111)
      ax.scatter(data.TowerLon, data.TowerLat, marker='.', alpha=0.3, linewidths=3, s=169)
      ax.scatter(centroids[:,0], centroids[:,1], marker='x', c='red', alpha=0.8, linewidths=3, s=169)
      ax.scatter(assignment2result[0], assignment2result[1], marker='x', c='black', alpha=0.8, linewidths=3, s=169)
  print(centroids)
...
#
# .. your code here ..
user1_no = df.In[0].tolist()
user_info = df.In.unique().tolist()
#
...
#
# .. your code here ..
#user1 = df[df.In == user1_no]
user1 = df[df.In == user_info[0]]
print(user1.In.iloc[0])
#
...
ax.set_title('Weekday Calls (>0am and <5pm)')
